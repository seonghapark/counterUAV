...
wav_repo = 'C://Users//승윤//Desktop//purdue//연구자료//counterUAV//wav_data'
...
def plot_specgram_1(file, i):#i번째 file
    ...
    librosa.output.write_wav('.//after_wav_data//'+file+'_'+'other'+".wav", other, samplingrate, norm=False)#saved mono
...
def cut_nsec(file_cut, n, i):
    ...
    sec = int(sec/5)
    p=0
    for time_slice in range(0,sec):
        start = time_slice*5862
        finis = time_slice*5862 + n*5862
        Y1 = np.array(Y[start:finis])
        librosa.output.write_wav('.//slice_wav_data//'+label+'_'+str(p)+''+str(i)+'.wav', Y1, 5862, norm=False)
        p +=1

# In[7]:


wav_repo = 'C://Users//승윤//Desktop//purdue//연구자료//counterUAV//new RNN//after_wav_data//'
wav_data = glob.glob(os.path.join(wav_repo,'**','*.wav'), recursive=True)
for i in range(len(wav_data)):
    cut_nsec(wav_data[i],5,i)

'''
def maketraintest(file_train):
    label = file_cut.split('_')[0]
...
wav_repo = 'C://Users//승윤//Desktop//purdue//연구자료//counterUAV//new RNN//slice_wav_data//'
wav_data = glob.glob(os.path.join(wav_repo,'**','*.wav'), recursive=True)
for i in range(len(wav_data)):
    maketraintest(wav_data[i])
'''
