...
def plot_specgram_1(file, i):#i번째 file
    ...
    for timeline in range(0,len(time_list[i])):#i번째 file vaild time list
        ...
        #tmp.extend(Y[time_start_index:time_finis_index])
        if others <len(time_list[i]):
            ...
            #other.extend(Y[other_start_index:other_finis_index])
    Y1 = tmp
    ...
    file = file.split('\\')[-1]
    file = file.split('.')[0]
    librosa.output.write_wav('C://Users//승윤//Desktop//purdue//연구자료//extractwav//cut_wav//stretched_1_'+file+'_'+label_list[i]+".wav", Y1, samplingrate, norm=False)#saved mono
    librosa.output.write_wav('C://Users//승윤//Desktop//purdue//연구자료//extractwav//cut_wav//stretched_1_'+file+'_'+'other'+".wav", other, samplingrate, norm=False)#saved mono
    return Y1
...
def cut_nsec(file_cut, time, i):
    ...
    if sec >= 5:
        for time_slice in range(0,sec,time):
            ...
            finis = time_slice*5862 + length*5*5862
            start = int(start)
            finis = int(finis)
            Y1 = np.array(Y[start:finis])
            if len(Y1) >= 5862*5*length:
