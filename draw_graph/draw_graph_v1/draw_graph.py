...
data_t = np.zeros((max_t, 200))
data_val = np.zeros((max_t, 200, y.shape[0]-1))
...
def animate(time):
	...
	split_line = np.pad(np.array(split_line), (0, 200 - int(data_tlen[max_t-1])), 'constant')
	...
	data_val = np.insert(data_val, max_t-1, np.zeros((200, y.shape[0]-1)),0)
