...
class inwav_handler(Thread):
    ...
    def run(self):
        while(True):
            ...
            q_raw.put(chunk) # A queue contains a chuck, which is an array length of Sampling rate per a second (fs)
            if chunk is None:
                break
            ...
            if (ii - spliter > 0) & (self.start[ii] == True) & (self.start[ii - 11 - spliter:ii - spliter].max() == False):  # if start[ii] is true and the mean of from start[ii-11] to start[ii-1] is zero (All False)
                for jj in range(ii - spliter, ii):
                    if (self.start[jj] == True) & (self.start[jj - 11:jj - 1].mean() == 0.0):
                        self.fsif[count, :] = self.rightarray[jj:jj + self.n]  # then copy rightarray from ii to ii+n and paste them to sif[count] --> sif[count] is a list
                        ...
                        self.fsif[count, :] = self.rightarray[jj + 1:jj + 1 + self.n]  # then copy rightarray from ii to ii+n and paste them to sif[count] --> sif[count] is a list
