...
class rmq_commumication():
    ...
    def get(self):
        ...
        sync, data = self.data_disassembler(bytearray(body))
        return sync, data
    ...
    def data_disassembler(self, data):
        ...
        # print('incomming data length: ', len(data))
        if (data[0] >> 5) == 6:  # lsB
            del data[0]
        if (len(data) % 2) == 1:
            del data[-1]
        # print('sorted data length: ', len(data))

        for i in range(0, len(data), 2):
            # data[i] is msB
            sync = data[i] & 0x20
            if sync == 32:
                self.sync.append(True)
            else:
                self.sync.append(False)

            self.sign = data[i] & 0x80
            integer_data = (data[i] & 0x1F) << 5 | (data[i+1] & 0x1F)
            if self.sign == 1:
                integer_data = integer_data * (-1)
            self.data.append(integer_data)

        return np.array(self.sync), np.array(self.data)
